...
int main(){
    ...
    int test_array[] = {
        ...
        }; //comment out a bunch of the array before directed testing, this algo is really slow, O(n^2) i belive
...
for (int i = 0; i < N - 1; i++) {
    ...
    return tb_return(test_array[0]);
    //return tb_return(pack_ptr((uint32_t)test_array, N, false)); //dont push without index, will fail verification
}
