...
Write-Host "Compiling quartus project...`n" -ForegroundColor Magenta

quartus_sh -t "$relativeScriptPath\build.tcl" $projectName $projectRev
if ($LASTEXITCODE -ne 0) {Write-Host "`n'quartus_sh -t "$relativeScriptPath\build.tcl" $projectName' threw an error`n" -ForegroundColor Red}
